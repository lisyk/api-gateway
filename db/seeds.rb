...
vcp_plan_mapping.each do |field, data_type|
  partner_field = DbService::PartnerField.create!(field_name: field, field_data_type: data_type, partner_id: partner.id)
...
vcp_to_vip.each do |vcp_field, vip_field|
  ...
  DbService::FieldMapping.create!(partner_field_id: vcp_record.id, vip_field_id: vip_record.id, translation_needed: true, mapping_document: 'wellness_plan')
end
